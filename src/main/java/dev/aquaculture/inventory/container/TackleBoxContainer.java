...
public class TackleBoxContainer extends AbstractContainerMenu {
  ...
  public Slot slotHook, slotBait, slotLine, slotBobber;
    ...
    if (this.tackleBox != null) {
      ...
      for (int column = 0; column < columns; ++column) {
        for (int row = 0; row < rows; ++row) {
          this.addSlot(new SlotItemHandler(tackleBoxCapability, 1 + row + column * columns, 8 + row * 18, 8 + column * 18) {
            @Override
            public boolean mayPlace(@Nonnull ItemStack stack) {
  ...
  public ItemStack quickMoveStack(@Nonnull Player player, int index) {
    ...
    if (slot != null && slot.hasItem()) {
      ...
      if (index < this.rows * this.columns) {
        if (!this.moveItemStackTo(slotStack, this.rows * this.columns, this.slots.size(), true)) {
          return ItemStack.EMPTY;
        }
      }
      else if (!this.moveItemStackTo(slotStack, 0, this.rows * this.columns, false)) {
        return ItemStack.EMPTY;
      }
  ...
  public void clicked(int slotId, int dragType, @Nonnull ClickType clickType, @Nonnull Player player) {
    if (slotId >= 0 && clickType == ClickType.PICKUP) {
      ...
      if (slot == this.slotBait) {
        ...
        if (slotHandler.mayPlace(mouseStack) && (slot.getItem().isDamaged() || slot.getItem().isEmpty() || slot.getItem().getItem() != mouseStack.getItem())) {
